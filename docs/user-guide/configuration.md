...
The VM automatically configures swap space based on your host operating system:

**Platform-Aware Defaults:**
- **Linux:** 2048 MB swap, swappiness 60 (standard settings)
- **macOS:** 1024 MB swap, swappiness 30 (optimized for macOS)
- **Windows:** 512 MB swap, swappiness 0 (minimal for performance)

You can override these defaults in your `vm.yaml`:

```yaml
vm:
  swap: 4096        # Set custom swap size in MB
  swappiness: 10    # Set custom swappiness (0-100)
```

**What is swappiness?**
- Controls how aggressively the kernel swaps memory to disk
- Range: 0 (avoid swap) to 100 (swap aggressively)
- Lower values keep more data in RAM, higher values swap more readily

### Port Binding (vm.port_binding)

**Default:** `127.0.0.1` (localhost only - secure)

By default, all services bind to `127.0.0.1` (localhost), making them accessible only from your machine. This is the recommended secure default.

**Example (default - secure):**
```yaml
vm:
  port_binding: 127.0.0.1  # Services only accessible from this machine
```

**LAN Access (use with caution):**

If you need to access services from other devices on your network (e.g., testing mobile apps, sharing with teammates):

```yaml
vm:
  port_binding: 0.0.0.0  # Services accessible from local network
```

**Security Warning:**
- Setting `0.0.0.0` exposes all services to your entire local network
- Any device on your WiFi/LAN can access your databases, APIs, etc.
- Only use on trusted networks (home/office, not public WiFi)
- Consider using VPN or SSH tunneling for remote access instead

**Use Cases:**
- `127.0.0.1` (default): Standard local development
- `0.0.0.0`: Testing from mobile devices, sharing with local team members

**Security Best Practices:**
1. Keep the default `127.0.0.1` unless you specifically need LAN access
2. If using `0.0.0.0`, ensure your firewall is enabled
3. Use strong database passwords (auto-generated by vm)
4. Don't use `0.0.0.0` on public networks
