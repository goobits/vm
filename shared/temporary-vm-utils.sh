...
EOF
  ...
  if [[ -n "$mounts" ]]; then
    ...
    for mount in "${MOUNT_ARRAY[@]}"; do
      ...
      local abs_source
      # Get absolute path, fallback to relative if resolution fails
      if ! abs_source=$(cd "$project_dir" && portable_readlink "$source" 2>/dev/null); then
        abs_source="$source"
      fi
      local target
...
handle_temp_command() {
  ...
  case "$temp_provider" in
    "docker")
      ...
      if ! check_docker_daemon_status; then
        log_error "Docker daemon is not running or not accessible"
        log_tip "Start Docker Desktop (if on macOS/Windows)"
        log_tip "Run: sudo systemctl start docker (if on Linux)"
        log_tip "Check permissions: groups | grep docker"
        exit 1
      fi
      ;;
    "vagrant")
  ...
  confirm_destroy() {
    ...
    if is_temp_vm_running "$container_name"; then
      ...
      if started_at=$(docker_cmd inspect "$container_name" --format='{{.State.StartedAt}}' 2>/dev/null); then
        ...
        if start_epoch=$(portable_date_to_epoch "$started_at") && current_epoch=$(date +%s); then
...
EOF
  ...
  for mount in "${MOUNT_ARGS[@]}"; do
    ...
    if [[ "${VM_DEBUG:-}" = "true" ]]; then
      ...
      echo "DEBUG handle_temp_command: full path would be: $(portable_readlink "$source" 2>/dev/null || echo "INVALID")" >&2
    fi
  ...
  if [[ -n "$existing_container" ]] && is_temp_vm_running "$existing_container"; then
    ...
    for mount in "${PROCESSED_MOUNTS[@]}"; do
      ...
      perm="${mount##*:}"
      # Get absolute path, fallback to relative if resolution fails
      if ! abs_source=$(portable_readlink "$source" 2>/dev/null); then
        abs_source="$source"
      fi
...
EOF
  ...
  for mount in "${PROCESSED_MOUNTS[@]}"; do
    ...
    perm="${mount##*:}"
    # Get absolute path
    REAL_PATH=$(portable_readlink "$source")
  ...
  if [[ "${AUTO_DESTROY:-}" = "true" ]]; then
    ...
    # Resolve temp directory path for security validation
    if REAL_TEMP_DIR=$(portable_readlink "$TEMP_PROJECT_DIR" 2>/dev/null); then
      : # Success - continue with validation
    else
      # Path resolution failed, use original path
      REAL_TEMP_DIR="$TEMP_PROJECT_DIR"
    fi
