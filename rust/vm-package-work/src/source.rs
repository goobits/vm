...
async fn run(command: &mut Command, operation: &str) -> WorkResult<Output> {
    let output = capture_output_async(command, operation, SOURCE_COMMAND_LIMITS)
        .await
        .map_err(|error| {
            if error.is_command_constraint() {
                WorkError::Conflict(error.to_string())
            } else {
                WorkError::Internal(error.to_string())
            }
        })?;
