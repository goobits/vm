...
use vm_provider::ImageConfig;

/// Tests for ImageConfig::parse_for_docker() with string specifications.
#[test]
fn test_docker_parses_image() {
    let spec = ImageSpec::String("ubuntu:24.04".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap();
    assert!(matches!(result, ImageConfig::DockerImage(_)));
    if let ImageConfig::DockerImage(image) = result {
...
fn test_docker_parses_image_node() {
    let spec = ImageSpec::String("node:20".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap();
    assert!(matches!(result, ImageConfig::DockerImage(_)));
    if let ImageConfig::DockerImage(image) = result {
...
fn test_docker_parses_image_with_registry() {
    let spec = ImageSpec::String("ghcr.io/user/myapp:latest".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap();
    assert!(matches!(result, ImageConfig::DockerImage(_)));
    if let ImageConfig::DockerImage(image) = result {
...
fn test_docker_parses_dockerfile_path_relative() {
    let spec = ImageSpec::String("./Dockerfile".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from("/workspace")).unwrap();
    assert!(matches!(result, ImageConfig::Dockerfile { .. }));
    if let ImageConfig::Dockerfile {
        path,
        context,
        args,
    } = result
...
fn test_docker_parses_dockerfile_path_parent() {
    let spec = ImageSpec::String("../Dockerfile".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from("/workspace/sub")).unwrap();
    assert!(matches!(result, ImageConfig::Dockerfile { .. }));
    if let ImageConfig::Dockerfile { path, context, .. } = result {
...
fn test_docker_parses_dockerfile_path_absolute() {
    let spec = ImageSpec::String("/path/to/Dockerfile".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from("/workspace")).unwrap();
    assert!(matches!(result, ImageConfig::Dockerfile { .. }));
    if let ImageConfig::Dockerfile { path, context, .. } = result {
...
fn test_docker_parses_dockerfile_extension() {
    let spec = ImageSpec::String("app.dockerfile".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from("/workspace")).unwrap();
    assert!(matches!(result, ImageConfig::Dockerfile { .. }));
    if let ImageConfig::Dockerfile { path, context, .. } = result {
...
fn test_docker_parses_dockerfile_extension_nested() {
    let spec = ImageSpec::String("docker/prod.dockerfile".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from("/workspace")).unwrap();
    assert!(matches!(result, ImageConfig::Dockerfile { .. }));
    if let ImageConfig::Dockerfile { path, context, .. } = result {
...
fn test_docker_parses_snapshot() {
    let spec = ImageSpec::String("@my-snapshot".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap();
    assert!(matches!(result, ImageConfig::Snapshot(_)));
    if let ImageConfig::Snapshot(name) = result {
...
fn test_docker_parses_snapshot_complex_name() {
    let spec = ImageSpec::String("@dev-snapshot-2024-01-15".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap();
    assert!(matches!(result, ImageConfig::Snapshot(_)));
    if let ImageConfig::Snapshot(name) = result {
...
#[test]
fn test_image_config_clone_docker_image() {
    let config = ImageConfig::DockerImage("ubuntu:24.04".to_string());
    let cloned = config.clone();
    assert!(matches!(cloned, ImageConfig::DockerImage(_)));
    if let ImageConfig::DockerImage(image) = cloned {
        assert_eq!(image, "ubuntu:24.04");
    }
}

#[test]
fn test_image_config_clone_dockerfile() {
    let config = ImageConfig::Dockerfile {
        path: PathBuf::from("/workspace/Dockerfile"),
        context: PathBuf::from("/workspace"),
        args: None,
    };
    let cloned = config.clone();
    assert!(matches!(cloned, ImageConfig::Dockerfile { .. }));
}

#[test]
fn test_image_config_clone_snapshot() {
    let config = ImageConfig::Snapshot("my-snapshot".to_string());
    let cloned = config.clone();
    assert!(matches!(cloned, ImageConfig::Snapshot(_)));
    if let ImageConfig::Snapshot(name) = cloned {
        assert_eq!(name, "my-snapshot");
    }
}

#[test]
fn test_image_config_debug_formatting() {
    let config = ImageConfig::DockerImage("ubuntu:24.04".to_string());
...
fn test_docker_image_vs_path_disambiguation() {
    ...
    let spec = ImageSpec::String("myregistry.com/path/to/image:tag".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap();
    assert!(matches!(result, ImageConfig::DockerImage(_)));

    // This should be treated as a Dockerfile path
    let spec = ImageSpec::String("./myregistry.com/Dockerfile".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap();
    assert!(matches!(result, ImageConfig::Dockerfile { .. }));
}
...
fn test_build_args_conversion() {
    ...
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from("/workspace")).unwrap();
    if let ImageConfig::Dockerfile { args, .. } = result {
        let args = args.unwrap();
...
fn test_empty_string_is_docker_image() {
    let spec = ImageSpec::String("".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap();
...
fn test_path_joins_with_base_dir() {
    let spec = ImageSpec::String("./custom.dockerfile".to_string());
    let result =
        ImageConfig::parse_for_docker(&spec, &PathBuf::from("/home/user/project")).unwrap();
    if let ImageConfig::Dockerfile { path, context, .. } = result {
...
fn test_docker_rejects_tart_base_image_names() {
    let spec = ImageSpec::String("vibe-tart-sequoia-base".to_string());
    let err = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap_err();
    assert!(err.to_string().contains("looks like a Tart image"));
}

#[test]
fn test_docker_rejects_cirrus_macos_tart_images() {
    let spec = ImageSpec::String("ghcr.io/cirruslabs/macos-sequoia-base:latest".to_string());
    let err = ImageConfig::parse_for_docker(&spec, &PathBuf::from(".")).unwrap_err();
    assert!(err.to_string().contains("looks like a Tart image"));
}
...
fn test_tart_rejects_dockerfile_string_paths() {
    for value in [
        ...
        let spec = ImageSpec::String(value.to_string());
        let err = ImageConfig::parse_for_tart(&spec).unwrap_err();
...
fn test_windows_absolute_path() {
    let spec = ImageSpec::String(r"C:\workspace\Dockerfile".to_string());
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(r"D:\project")).unwrap();
    if let ImageConfig::Dockerfile { path, .. } = result {
...
fn test_windows_absolute_build_spec() {
    ...
    let result = ImageConfig::parse_for_docker(&spec, &PathBuf::from(r"E:\base")).unwrap();
    if let ImageConfig::Dockerfile { path, context, .. } = result {
