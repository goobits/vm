...
fn is_safe_to_restart(project_name: &str) -> (bool, u32) {
    match count_active_ssh_sessions(project_name) {
        ...
        Err(e) => {
            ...
            debug!("Could not read SSH session state: {}. Assuming unsafe to restart.", e);
...
mod tests {
    ...
    fn test_worktrees_match() {
        let worktrees = vec!["/path/to/worktree1".to_string(), "/path/to/worktree2".to_string()];
        let mounts = vec!["/path/to/worktree1".to_string(), "/path/to/worktree2".to_string(), "/path/to/other".to_string()];
        assert!(worktrees_match(&worktrees, &mounts));

        let worktrees = vec!["/path/to/worktree1".to_string(), "/path/to/worktree3".to_string()];
        assert!(!worktrees_match(&worktrees, &mounts));
