...
impl VmState {
    ...
    pub fn save(&self, project_name: &str) -> Result<()> {
        let state_path = Self::get_state_path(project_name)?;
        let state_dir = state_path.parent().ok_or_else(|| VmError::Internal("Could not get parent directory for state file".to_string()))?;
    ...
    fn get_state_path(project_name: &str) -> Result<PathBuf> {
        let home_dir = dirs::home_dir().ok_or_else(|| VmError::Internal("Could not get home directory".to_string()))?;
