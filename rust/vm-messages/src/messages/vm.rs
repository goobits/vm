...
pub const VM_MESSAGES: VmMessages = VmMessages {
    ...
    create_troubleshooting: "\n❌ Failed to create '{name}'\n   Error: {error}\n\n💡 Try:\n   • List environments: vm ls\n   • Recreate with: vm rm {name} --force && vm run linux as {name}\n   • Review provider-specific logs if available",
    ...
    // VM SSH
    ssh_connecting: "\n🔗 Connecting to '{name}'...",
    ssh_disconnected: "\n👋 Disconnected from '{name}'\n💡 Reconnect with: vm shell {name}\n",
    ssh_vm_not_found: "\n🔍 VM '{name}' doesn't exist",
    ssh_create_prompt: "\nWould you like to create it now? (y/N): ",
    ssh_creating: "\n🚀 Creating '{name}'...\n\n  ✓ Preparing base environment\n  ✓ Setting up workspace\n  ✓ Configuring network\n  ✓ Starting environment\n  ✓ Running initial provisioning",
    ssh_create_success: "\n✅ Created successfully\n\n🔗 Connecting to '{name}'...",
    ssh_create_failed: "\n❌ Failed to create '{name}'\n   Error: {error}\n\n💡 Try:\n   • List environments: vm ls\n   • Review provider-specific logs if available\n   • Run: vm run linux as {name}",
    ssh_not_running: "\n⚠️  VM '{name}' is not running",
    ssh_connection_lost: "\n⚠️  Lost connection to VM\n💡 Check running environments: vm ls",
    ssh_session_ended: "\n⚠️  Session ended unexpectedly\n💡 Check environments: vm ls",
    ssh_start_hint: "\n💡 Start the environment with: vm run linux\n💡 Then reconnect with: vm shell",
    ssh_start_prompt: "\nWould you like to start it now? (Y/n): ",
    ssh_start_aborted: "\n❌ Shell connection aborted\n💡 Start the environment manually with: vm run linux",
    ssh_starting: "\n🚀 Starting '{name}'...",
    ssh_start_failed: "\n❌ Failed to start '{name}': {error}\n\n💡 Try:\n   • List environments: vm ls\n   • Review provider-specific logs if available\n   • Recreate environment: vm rm {name} --force && vm run linux as {name}",
    ssh_reconnecting: "✅ Started successfully\n\n🔗 Reconnecting to '{name}'...",

    // VM Destroy Enhanced (Cross-Provider)
    destroy_cross_no_instances: "No instances found to remove",
    destroy_cross_list_header: "Instances to remove:",
    destroy_cross_list_item: "  {name} ({provider})",
    destroy_cross_confirm_prompt: "\nAre you sure you want to remove {count} instance(s)? (y/N): ",
    destroy_cross_cancelled: "Remove operation cancelled",
    destroy_cross_progress: "Removing {name} ({provider})...",
    destroy_cross_success_item: "  ✅ Successfully removed {name}",
    destroy_cross_failed: "  ❌ Failed to remove {name}: {error}",
    ...
    doctor_config_not_found_hint: "   💡 Run 'vm run linux' to create a configuration file",
    ...
    doctor_auth_not_responding_hint: "   💡 Check with: vm secret status",
    ...
    doctor_pkg_not_responding_hint: "   💡 Check with: vm system registry status",
