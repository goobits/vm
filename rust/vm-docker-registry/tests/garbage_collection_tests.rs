...
async fn test_garbage_collection_frees_space() {
    ...
    server::start_registry().await.expect("Failed to start registry");
    ...
    run_docker_command(&["tag", image, local_image]).await.unwrap();
    ...
    let response = client.head(manifest_url)
        .header("Accept", "application/vnd.docker.distribution.manifest.v2+json")
        .send()
        .await
        .expect("Failed to get manifest digest");

    let digest = response.headers().get("Docker-Content-Digest").expect("No digest found").to_str().unwrap();

    let delete_url = format!("http://localhost:6000/v2/test-gc-image/manifests/{}", digest);
    client.delete(&delete_url).send().await.expect("Failed to delete manifest");

    // 4. Run garbage collection
    let result = server::garbage_collect(true).await;
    assert!(result.is_ok(), "Garbage collection should run without errors");

    let gc_result = result.unwrap();
    assert!(gc_result.bytes_freed > 0, "Garbage collection should free some space");
    assert_eq!(gc_result.errors.len(), 0, "There should be no errors during garbage collection");

    // 5. Clean up
    server::stop_registry().await.expect("Failed to stop registry");
